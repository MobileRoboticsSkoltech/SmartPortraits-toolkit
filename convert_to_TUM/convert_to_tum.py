...
def pointcloudify_depth(depth, intrinsics, dist_coeff, undistort=True):
    ...
    # 3 x H * W
    local_grid = inv_undist_intrinsics @ grid.reshape(-1, 3).transpose()
...
def project_pcd_to_depth(pcd, undist_intrinsics, img_size, config_file):
    h, w = img_size
    points = np.asarray(pcd.points)
    d = points[:, 2]  # np.linalg.norm(points, axis=1)
    normalized_points = points / np.expand_dims(points[:, 2], axis=1)
    proj_pcd = np.round(undist_intrinsics @
                        normalized_points.T).astype(np.int64)[:2].T
    proj_mask = (proj_pcd[:, 0] >= 0) & (proj_pcd[:, 0] < w) & (
        proj_pcd[:, 1] >= 0) & (proj_pcd[:, 1] < h)
    proj_pcd = proj_pcd[proj_mask, :]
    d = d[proj_mask]
    pcd_image = np.zeros((config_file["res_h"], config_file["res_w"]))
...
def align_rgb_depth(rgb, depth, roi, config_file):
    ...
    rgb_cnf = np.load(os.path.join(
        curr_dir, config_file["rgb_intristics"]), allow_pickle=True).item()
    ...
    aligned_depth = project_pcd_to_depth(
        pcd, rgb_cnf['undist_intrinsics'], rgb.shape[:2], config_file)
...
def process_pair(rgbd_pair):
    rgb_image = cv.imread(os.path.join(
        smartphone_folder, str(rgbd_pair[0]) + '.png'))
    depth_array = np.load(os.path.join(azure_depth_folder, str(
        rgbd_pair[1]) + '.npy'), allow_pickle=True)

    rgb_image_aligned, depth_array_aligned = align_rgb_depth(
        rgb_image, depth_array, (0, 0, config["res_w"], config["res_h"]), config)

    # Save rgb as 8-bit png
    cv.imwrite(os.path.join(final_folder, 'rgb', str(
        rgbd_pair[0]) + '.png'), rgb_image_aligned)
    ...
    imageio.imwrite(os.path.join(final_folder, 'depth', str(
        rgbd_pair[1]) + '.png'), depth_array_aligned)
...
#            continue
    ...
    print('Depth timestamps from {1} to {2} (cnt {0})'.format(
        len(depth_ts), depth_ts[0], depth_ts[-1]))
    print('RGB timestamps from {1} to {2} (cnt {0})'.format(
        len(rgb_ts), rgb_ts[0], rgb_ts[-1]))
